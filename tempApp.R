...
pkgnames <- c("tidyverse","shiny", "shinyjs","DBI","jsonlite","shinydashboard", "shiny.router", "bs4Dash")
loadPkgs(pkgnames)
...
ui = fluidPage(
  shinyjs::useShinyjs(),
    ...
    sidebar = dashboardSidebar(
      ...
      sidebarMenu(
        actionButton("register", "Register", icon("user"), status= "primary" ),
        actionButton("login", "Login", icon("user-check"), status = "primary")
      )
    ),
    
    #controlbar = dashboardControlbar(),
    footer = dashboardFooter(),
    body = dashboardBody(gameInterface)
  )
